...
- Resizable surfaces (Settings, onboarding) use the scalable `TypeRamp` (Dynamic Type);
  the palette is a fixed-size HUD with tuned point sizes (with `@ScaledMetric`, clamped to xxLarge).
...
- **Terminal permissions (TCC):** Terminal.app/iTerm2 use AppleScript → **Automation**;
  Ghostty types the command → **Accessibility**; CLI terminals (kitty/Alacritty/WezTerm) need neither.
...
- **Code signing (important):**
  ...
  - **SPM ships flat resource bundles** (no Info.plist) → codesign calls them "unsuitable";
    `package.sh` injects a minimal Info.plist and signs them. Sparkle's nested XPC/Updater.app
