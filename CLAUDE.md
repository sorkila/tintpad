...
`Resources/appicon-raw.jpg`, regenerated by `Scripts/make-icon.swift` which also
emits favicon-32), and **a permission failure is no longer a dead end**:
`TerminalLaunchError.permissionNeeded(summary:remedy:pane:)` carries the System
Settings pane, the palette holds it as a pending state (short red line, Return
opens the pane via `PrivacyPane.open()` which fires the AX prompt first, Esc
cancels, all the same cancel paths as the dangerous confirm), and the error copy
plus the site FAQ cover the **stale-grant trap** (macOS keys TCC grants to the
code signature, so a grant made to a differently signed build shows enabled in
System Settings yet silently fails — remove the entry and re-add it). Born live:
the developer's own dev-build grant went stale and the one-line status read as
"nothing happens".
